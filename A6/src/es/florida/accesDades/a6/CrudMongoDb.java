package es.florida.accesDades.a6;
...
public class CrudMongoDb {
	...
	public static void llegir(MongoDatabase db, MongoCollection<Document> coleccio) {
		...
		System.out.println("\n> MOSTREM EL DOCUMENT O DOCUMENTS AMB TOTES LES DADES\n");
		while(cursor.hasNext()) {
			System.out.println(cursor.next().toJson());
		}
		
		System.out.println("\n> FILTREM DADES\n");
		cursor = coleccio.find(Filters.eq("artista", "a-ha")).iterator();
		while(cursor.hasNext()) {
			System.out.println(cursor.next().toJson());
		}
	...
	public static void main(String[] args) {
		...
		MongoDatabase mongoDb = mongoClient.getDatabase("artistas");
		MongoCollection<Document> coleccio = mongoDb.getCollection("albumes");
